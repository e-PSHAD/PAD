...
function(
    ...
    var buildPagingBarTemplateContextKnownLength = function(numberOfItems, itemsPerPage) {
        ...
        /*** PADPLUS
         * It seems itemsPerPage can be either a number (see #createFromAjax call in random_question_form_preview.js)
         * or a {value: <number>, active: <boolean>} object in myoverview and timeline blocks.
         * This fix takes both types into account, especially when using PagedContentFactory.createWithTotalAndLimit,
         * which calls this method if (total) numberOfItems is given.
         */
        var itemsPage = ('value' in itemsPerPage) ? itemsPerPage.value : itemsPerPage;
        // When user has less than 12 courses, itemsPerPage sole option is {value: 0}.
        // In this case avoid divide-by-zero error by using default 12 items per page.
        itemsPage = (itemsPage > 0) ? itemsPage : 12;
        var numberOfPages = calculateNumberOfPages(numberOfItems, itemsPage);
